...
# Call init_doctors_table on startup
init_doctors_table()
...
def get_patient_notes(patient_id):
    try:
        conn = get_db_connection()
        notes = conn.execute('SELECT note, date FROM patient_notes WHERE patient_id = ?', (patient_id,)).fetchall()
        conn.close()
        print(f"Fetched {len(notes)} notes for patient {patient_id}")
        return jsonify([{'note': row['note'], 'date': row['date']} for row in notes])
    except sqlite3.OperationalError as e:
