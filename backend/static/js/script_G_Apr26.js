                ...
                fetch(`/api/patient_notes/${patientId}`, { credentials: 'include' })
                    ...
                    .then(data => {
                        if (data && data.length > 0) {
                            ...
                            notesList.innerHTML = data.map(note => `<li>${note.date || 'No date available'}: ${note.note}</li>`).join('');
                        } else {
            ...
            if (event.results[0].isFinal) {
                ...
                if (patientId) {
                    fetch(`/transcribe/${patientId}`, {
                        ...
                        .then(response => {
                            if (!response.ok) throw new Error(`Failed to save transcription: ${response.status}`);
                            return response.json();
                        })
                        .then(data => {
                            console.log("Transcription saved:", data);
                            // Refresh patient notes immediately
                            const notesList = document.getElementById('patient-notes');
                            if (notesList) {
                                fetch(`/api/patient_notes/${patientId}`, { credentials: 'include' })
                                    .then(response => {
